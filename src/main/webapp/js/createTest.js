...
function parseTest(url){
    ...
    let time = (Number.parseInt(minute) * 60) + Number.parseInt(second);
    if (!time){
        time = 0;
    }
    ...
    for (let i = 0; i < questionsElement.length; i++) {
        ...
        for (let j = 0; j < answerElement.length; j++) {
            ...
            answers.push(makeAnswer(answerValue,trueAnswer))
        }
        const questionBean = makeQuestionBean(makeQuestion(questionName,questionType),answers);
        questionBeans.push(questionBean)
    }

    const test = makeTest(testName,time,complexityId,subjectId)
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            urlSearchParams.set("command", "update_test_page");
            urlSearchParams.set("id", data.id);
...
$(document).on('click', '.addAnswer', function () {
    ...
    const newAnswer = $(" <div class=\"input-group  mt-2 mb-2 answer\">\n" +
        "                <div class=\"input-group-text\">\n" +
...
function makeQuestion(name,questionType){
    return{
        name: name,
        questionTypeId:questionType,
    }
}
function makeTest(name,time,complexityId,subject){
    return{
        name: name,
        time: time,
...
function makeAnswer(name,trueAnswer){
    return{
        name: name,
