...
/**
 * Functional command for create test
 *
...
public class CreateTestCommand extends Command {
    ...
    public String execute(HttpServletRequest request, HttpServletResponse response) throws ServletException {
        ...
        try {
            ...
            testManager.createTestBean(testBean);
            LOG.debug("Create test success");
            response.setContentType("application/json");
            response.setCharacterEncoding("UTF-8");
            response.getOutputStream().print("{\"id\":\""+ testBean.getTest().getId() +"\"}");
        } catch (IOException e) {
            ...
            LOG.error("DB error no create testBean",e);
            try {
                response.sendError(500);
            } catch (IOException ex) {
                ex.printStackTrace();
